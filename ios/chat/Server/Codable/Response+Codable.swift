...
extension Moya.Response {
    ...
    /// - Returns: 解析成功后的对象
    /// - Throws: 解析失败时的错误
    public func mapObject<T: BaseMappable>(_ type: T.Type, keyPath: String?, decrypt: Bool) throws -> T {
        let json = try validationJSON(decrypt: decrypt)
    ...
    public func mapPage<T: Mappable>(_ type: T.Type, keyPath: String?, decrypt: Bool) throws -> List<T> {
        ...
        if let keyPath = keyPath {
            ...
            let countKeyPath = single ? CodableKey.List.totalCount :
            ...
            guard
                let array = json.value(forKeyPath: keyPath) as? NSArray,
                let totalCount = json.value(forKeyPath: countKeyPath) as? Int else
            {
                throw APIError.objectMappingFailed
            }
            list = array
            ...
            guard
                let array = json[CodableKey.List.list] as? NSArray,
                let totalCount = json[CodableKey.List.totalCount] as? Int else
            {
                throw APIError.objectMappingFailed
            }
            list = array
        ...
        return List(datas: listInfo, count: count, next: next)
    }
...
private extension Moya.Response {
    ...
    func validationJSON(decrypt: Bool) throws -> NSDictionary {
        ...
        if code == CodableKey.loginExpiredCode {
        ...
        guard code == CodableKey.successCode else {
