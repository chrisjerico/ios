...
//        kArrowWidth = 15;
        ...
        _contentColor = [UIColor colorWithRed:47/255.0 green:57/255.0 blue:77/255.0 alpha:1];
        _separatorColor = [UIColor lightGrayColor];
...
- (void)dismiss
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ybPopupMenuDidSelectedAtIndex:ybPopupMenu:)]) {
        ...
        [self.delegate ybPopupMenuDidSelectedAtIndex:-1 ybPopupMenu:self];
    }
...
+ (instancetype)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuSize:(CGSize)itemSize delegate:(id<YBPopupMenuDelegate>)delegate
{
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemSize delegate:delegate];
    [popupMenu showAtPoint:point];
    return popupMenu;
}

+ (instancetype)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuSize:(CGSize)itemSize delegate:(id<YBPopupMenuDelegate>)delegate
{
    YBPopupMenu *popupMenu = [[YBPopupMenu alloc] initWithTitles:titles icons:icons menuWidth:itemSize delegate:delegate];
...
//    cell.titleLabel.text = [NSString stringWithFormat:@"%@",_titles[indexPath.row]];
