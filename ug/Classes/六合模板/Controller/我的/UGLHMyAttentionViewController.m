...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_mySegment.selectedSegmentIndex == 0) {
        ...
        [subButton(@"取消专家Btn") addBlockForControlEvents:UIControlEventTouchUpInside block:^(__kindof UIControl *sender) {
            ...
            __weakSelf_(__self);
              [NetworkManager1 lhcdoc_followPoster:model.posterUid followFlag:NO].successBlock = ^(CCSessionModel *sm, id responseObject) {[__self.tableView.mj_header beginRefreshing];
              };
        }];
        return cell;
    } else {
        ...
        [subButton(@"取消帖子Btn") addBlockForControlEvents:UIControlEventTouchUpInside block:^(__kindof UIControl *sender) {
            ...
            __weakSelf_(__self);
              [NetworkManager1 lhcdoc_doFavorites:model.cid type:2 favFlag:NO].successBlock = ^(CCSessionModel *sm, id responseObject) {[__self.tableView.mj_header beginRefreshing];
              };
        }];
        return cell;
    }
