...
@property (weak, nonatomic) IBOutlet UIView *segeMentContainerView;
@property (nonatomic, assign) NSInteger segmentIndex;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *segmentHeight;

@property (nonatomic, strong) NSMutableArray * dwdGameData;
...
-(void)viewDidLayoutSubviews{
	...
	//	}];
	//	[self.segmentView mas_remakeConstraints:^(MASConstraintMaker *make) {
	//		make.edges.equalTo(self.segeMentContainerView);
	//	}];
	...
	[self.rightStackView  mas_remakeConstraints:^(MASConstraintMaker *make)
	 {
		...
		make.top.equalTo(self.segeMentContainerView.mas_bottom);
	}];
...
- (void)viewDidLoad {
	...
	[self.segeMentContainerView addSubview:self.segmentView];
	...
	self.segmentView.segmentIndexBlock = ^(NSInteger row) {
		...
		for (UGGameplayModel *model in weakSelf.gameDataArray) {
			if ([@"定位胆" isEqualToString:model.name]) {
				model.list = weakSelf.dwdGameData[weakSelf.segmentIndex];
			}
		}
...
//定位胆玩法数据处理
- (void)generateDWDData {
	for (UGGameplayModel *model in self.gameDataArray) {
		if ([@"定位胆" isEqualToString:model.name]) {
			[self.dwdGameData removeAllObjects];
			NSMutableArray * titleArray = [NSMutableArray array];
			for (UGGameplaySectionModel *group in model.list) {
				
				[titleArray appendObject:group.alias];
				
				NSMutableArray * oddsSubArray = [NSMutableArray array];
				if ([@"复式" isEqualToString:group.alias]) {
					NSArray * sectionNames = @[@"百定位", @"十定位", @"个定位"];
					
					for (int i = 0; i < 3; i++) {
						UGGameplaySectionModel * newSection = group.copy;
						...
						newSection.list = array.copy;
						newSection.name = sectionNames[i];
						[oddsSubArray appendObject:newSection];
					}
					
					
				} else if ([@"组选3" isEqualToString:group.alias]) {
					NSArray * sectionNames = @[@"二重号", @"单号"];
					for (int i = 0; i < 2; i++) {
						UGGameBetModel *play = group.list.firstObject;
						NSMutableArray *array = [NSMutableArray array];
						UGGameplaySectionModel * newSection = group.copy;
				...
				} else if ([@"组选6" isEqualToString:group.alias]) {
					UGGameplaySectionModel * newSection = group.copy;
...
- (NSMutableArray *)dwdGameData {
	if (!_dwdGameData) {
		_dwdGameData = [NSMutableArray array];
	}
	return _dwdGameData;
}
...
- (void)initBetCollectionView {
	...
	UICollectionView *collectionView = ({
		...
		if ([CMCommon isPhoneX]) {
			height = UGScerrnH - 88 - 83 - 114 - 40;
		}else {
			height = UGScerrnH - 64 - 49 - 114 - 40;
		}
