                ...
                if (!UserI.isTest && SysConf.chatFollowSwitch && (amountfloat >= webAmountfloat)) {
                   ...
                   [LEEAlert alert].config
                       ...
                       .LeeAction(@"分享", ^{//跳到聊天界面，把分享数据传过去
                           ...
                           NSString *url = _NSString(@"%@%@%@&color=%@&back=hide&loginsessid=%@&logintoken=%@&tag=3&from=app", baseServerUrl, @"/dist/index.html#/home?roomId=0&roomName=", SysConf.chatRoomName, Skin1.navBarBgColor.hexString, [UGUserModel currentUser].sessid, [UGUserModel currentUser].token);
                           NSLog(@"url = %@",url);
