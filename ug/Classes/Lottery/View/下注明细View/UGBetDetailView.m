...
- (void)setDataArray:(NSArray<UGGameBetModel *> *)dataArray {
	...
	if ([@"连尾" isEqualToString:bet.typeName] || [@"连肖" isEqualToString:bet.typeName]) {
		NSArray *pickArrs = [[NSArray alloc] init];
		if ([@"二连尾" isEqualToString:bet.title] || [@"二连肖" isEqualToString:bet.title]) {
			pickArrs = [CMCommon pickPermutation:2 totalNum:array.count];
		} else if ([@"三连尾" isEqualToString:bet.title] ||[@"三连肖" isEqualToString:bet.title]){
			pickArrs = [CMCommon pickPermutation:3 totalNum:array.count];
		} else if ([@"四连尾" isEqualToString:bet.title] || [@"四连肖" isEqualToString:bet.title]){
