...
class FansListVC: BaseVC {
	...
	override func viewDidLoad() {
		...
		Observable.merge(tableView.mj_header.rx.refreshing.asObservable(), shouldRefresh.asObservable()).flatMap { [weak self] _ in
			return momentsAPI.rx.request(MomentsTarget.fansList(page: 1, rows: 20)).mapObject(MomentUserDataModel.self).do( onError: { (error) in
		...
		tableView.mj_footer.rx.refreshing.flatMap { [weak self] _ ->  Observable<MomentUserDataModel>  in
			...
			return momentsAPI.rx.request(MomentsTarget.fansList(page: weakSelf.page, rows: weakSelf.rows)).asObservable().mapObject(MomentUserDataModel.self).do( onError: {  (error) in
...
extension FansListVC: UITableViewDelegate {
