...
public class GameFragment extends BaseFragment {
    ...
    public void initView(View view) {
        if (signViewPager != null)
            signViewPager.setObjectForPosition(view, 2);
            initRecyclerView();
    }
...
//        lotteryticketAdapter = new LotteryticketAdapter(game, getActivity());
        ...
        lotteryticketAdapter.setListener(new LotteryticketAdapter.OnClickListener() {
            ...
            public void onClickListener(View view, int position) {
                if(game.get(position).getIsPopup()==0){
                    ...
                    TDialog mTDialog = new TDialog(getActivity(), TDialog.Style.Center, array, hint_list[0],
                            hint_list[1]+ game.get(position).getTitle()+
                                    hint_list[2],""
                    ...
                    mTDialog.show();
                }else if(game.get(position).getIsPopup()==1){
                    Intent intent=new Intent();
                    intent.putExtra("id",game.get(position).getId());
                    intent.putExtra("name",game.get(position).getCategory());
...
//            startActivity(new Intent(getContext(), LoginActivity.class));
        ...
        OkGo.<String>get(URLDecoder.decode(Constants.BaseUrl()+Constants.GOTOGAMEREAL +String.format(UrlModel, SecretUtils.DESede(game.get(position).getId()),SecretUtils.DESede(token))+"&sign="+SecretUtils.RsaToken()))//
                .tag(this)//
