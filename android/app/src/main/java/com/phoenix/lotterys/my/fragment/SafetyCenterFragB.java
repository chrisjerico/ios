...
public class SafetyCenterFragB extends BaseFragments implements BaseRecyclerAdapter.
    ...
    @BindView(R2.id.goto_binding_tex)
    TextView gotoBindingTex;
    @BindView(R2.id.goto_setting_lin)
    LinearLayout gotoSettingLin;
    @BindView(R2.id.modification_lin)
    LinearLayout modificationLin;
    @BindView(R2.id.stop_google_tex)
    TextView stopGoogleTex;
    @BindView(R2.id.tv01)
    TextView tv01;
    @BindView(R2.id.tv02)
    TextView tv02;
    ...
    @BindView(R2.id.blake_lin)
    LinearLayout blakeLin;
    ...
    FragmentUtilAct activity;
    boolean isHide = false;

    private int type;

    @SuppressLint("ValidFragment")
    public SafetyCenterFragB(boolean isHide) {
        super(R.layout.safety_center_frag_b, true, true);
        this.isHide = isHide;
    }
    ...
    public void initView(View view) {
        ...
        if (!isHide) {
            activity = (FragmentUtilAct) getActivity();
        ...
        titlebar.setLeftIconOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isBla){
                    ...
                    startActivity(new Intent(getContext(), BlackMainActivity.class));
                }
            }
        });
...
//        Uiutils.setBarStye(titlebar, getActivity());
        ...
        countryImg.setColorFilter(getResources().getColor(R.color.color_white));
        provinceImg.setColorFilter(getResources().getColor(R.color.color_white));
        cityImg.setColorFilter(getResources().getColor(R.color.color_white));
    ...
    private void isBlack() {
        if (null!=getArguments()) {
            ...
            isBla = getArguments().getBoolean("isBla");
        }

        tabIndex = type;
        if (isBla) {
            blakeLin.setVisibility(View.VISIBLE);
            mainRadio.setVisibility(View.GONE);
            initImageView();
            setLister(type);
        } else {
            mainRadio.setVisibility(View.VISIBLE);
            blakeLin.setVisibility(View.GONE);
        }
    }

    private boolean isBla;

    private void seleIndex(int id) {
        switch (id) {
            case 0:
            ...
            case 1:
                ...
                verificationLin.setVisibility(View.GONE);
                googleBindingLin.setVisibility(View.GONE);
                if (null != userInfo && null != userInfo.getData() && userInfo.getData().isHasFundPwd()) {
                    modificationLin.setVisibility(View.VISIBLE);
                } else {
                    gotoSettingLin.setVisibility(View.VISIBLE);
                }
                commonLoginSitesLin.setVisibility(View.GONE);
                secondaryValidationLin.setVisibility(View.GONE);
    ...
    private void setGoole() {
        if (null != userInfo && null != userInfo.getData() && userInfo.getData().isBindGoogleVerifier())
            isBind = true;
    }
    ...
    private void setTopsPop() {
        View tipsView = LayoutInflater.from(getActivity()).inflate(R.layout.tips_pop_b,
                null);
...
//                .create();
    ...
    @OnClick({R.id.country_lin, R.id.province_lin, R.id.city_lin, R.id.new_password_but,
            ...
            , R.id.verification_go_back_tex, R.id.verification_ok_tex, R.id.stop_google_tex, R.id.goto_binding_tex,
            R.id.tv01, R.id.tv02, R.id.tv03, R.id.tv04})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.added_city_commit_but:
                Log.e("addedList==",addedList.toString());
                if (addedList.size() > 0) {
                    for (AddedCityBean.DataBean dataBean : addedList) {
                        if (null != addedCityBean && StringUtils.equals(addedCityBean.getCity(), dataBean.getCity())
                                && StringUtils.equals(addedCityBean.getCountry(), dataBean.getCountry())) {
...
//                updata = new UpdataUtil(getActivity(), apkUrl, 1);
                ...
                if (!isHide)
                    ...
                    ((MainActivity) getActivity()).setDown(apkUrl);
                break;
            case R.id.next_tex:
                secondaryValidationLin.setVisibility(View.GONE);
...
//        float toX = lineWidth * position;
        ...
        if (position==current_index){
            titles[position].setTextColor(getResources().getColor(R.color.fount1));
        }else{
            titles[position].setTextColor(getResources().getColor(R.color.fount1));
...
////                                        qrCodeImg.setImageBitmap(bitmap);
    ...
    private void getData() {
        ...
        if (null != addedCityBean && !StringUtils.isEmpty(addedCityBean.getCountry())){
