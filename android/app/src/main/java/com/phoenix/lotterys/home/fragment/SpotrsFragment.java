...
public class SpotrsFragment extends BaseFragment {
    ...
    public void initView(View view) {
        if (signViewPager != null)
            signViewPager.setObjectForPosition(view, 4);
        initRecyclerView();
    }
...
//        lotteryticketAdapter = new LotteryticketAdapter(sport, getActivity());
        ...
        lotteryticketAdapter.setListener(new LotteryticketAdapter.OnClickListener() {
            ...
            public void onClickListener(View view, int position) {
                if(sport.get(position).getIsPopup()==0){
                    ...
                    TDialog mTDialog = new TDialog(getActivity(), TDialog.Style.Center, array, hint_list[0],
                            hint_list[1]+ sport.get(position).getTitle()+
                                    hint_list[2],""
                    ...
                    mTDialog.show();
                }else if(sport.get(position).getIsPopup()==1){
                    Intent intent=new Intent();
                    intent.putExtra("id",sport.get(position).getId());
                    intent.putExtra("name",sport.get(position).getCategory());
                    intent.putExtra("title",sport.get(position).getTitle());
...
//            startActivity(new Intent(getContext(), LoginActivity.class));
        ...
        OkGo.<String>get(URLDecoder.decode(Constants.BaseUrl()+Constants.GOTOGAMEREAL +String.format(UrlModel, SecretUtils.DESede(sport.get(position).getId()),SecretUtils.DESede(token))+"&sign="+SecretUtils.RsaToken()))//
                .tag(this)//
