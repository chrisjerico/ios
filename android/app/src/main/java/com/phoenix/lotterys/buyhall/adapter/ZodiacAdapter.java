...
public class ZodiacAdapter extends RecyclerView.Adapter<ZodiacAdapter.ViewHolder> {
    List<Zodiac> list;
    private Context context;
    private LayoutInflater inflater;
    private ZodiacAdapter.OnClickListener onClickListener;
    private SparseBooleanArray mCheckStates = new SparseBooleanArray();
    private final ConfigBean config;
    public ZodiacAdapter(List<Zodiac> list, Context context) {
        this.list = list;
    ...
    public void setListener(ZodiacAdapter.OnClickListener onClickListener) {
        this.onClickListener = onClickListener;
    }

    public interface OnClickListener {
        void onClickListener(View view, int position, List<Zodiac> list);
    }

    @NonNull
    @Override
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = inflater.inflate(R.layout.zodiac_item, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
        ...
        holder.cb.setText(list.get(position).getZodiac());
...
//            list.get(position).setSelect(false);
    ...
    public void ZodiacClearItem(String zod) {
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).getZodiac().equals(zod)) {
    ...
    public void ZodiacSelectItem(String zod) {
        Log.e("getzod","getzod"+zod);
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).getZodiac().equals(zod)) {
                    list.get(i).setSelect(true);
                    Log.e("getzod1","getzod"+zod);
