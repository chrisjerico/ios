...
//import static com.phoenix.lotterys.util.Constants.SLIDECODE;
...
public class BlackLoginActivity extends BaseActivitys implements AdvancedWebView.Listener {
    ...
    public BlackLoginActivity() {
        super(R.layout.black_activity_login);
    }
...
//        gameWebView.setVisibility(View.VISIBLE);
    ...
    private void siteName() {
        ConfigBean config = (ConfigBean) ShareUtils.getObject(this, SPConstants.CONFIGBEAN, ConfigBean.class);
        if (config!=null&&config.getData()!=null&&config.getData().getWebName() != null) {
    ...
    @OnClick({R.id.bt_login, R.id.bt_regedit, R.id.bt_playgame, R.id.bt_gotohome, R.id.tv_close, R.id.tv_forget})
...
//                startActivity(new Intent(this, RegeditActivity.class));
                EvenBusUtils.setEvenBus(new Even(EvenBusCode.SHOWFRAMNET, "/regedit"));
                finish();
                break;
            ...
            case R.id.tv_close:
                ...
                break;
            case R.id.tv_forget:
                SkipGameUtil.SkipNavig("7", "4", "", this, "", "", "");
                break;
        }
    }
    ...
    private boolean questLogin() throws UnsupportedEncodingException {
        ...
        if (mEtUser == null || mEtUser.length() == 0) {
            ToastUtils.ToastUtils(getResources().getString(R.string.inputuser), BlackLoginActivity.this);
            slideCode();
            return false;
        } else if (mEtUser.length() < 4) {
            ToastUtils.ToastUtils(getResources().getString(R.string.inputuser9), BlackLoginActivity.this);
            slideCode();
            return false;
        }
        if (mEtPassword == null || mEtPassword.length() == 0) {
            ToastUtils.ToastUtils(getResources().getString(R.string.inputpwd), BlackLoginActivity.this);
            slideCode();
            return false;
        } else if (mEtPassword.length() < 6) {
            ToastUtils.ToastUtils(getResources().getString(R.string.inputpwd8), BlackLoginActivity.this);
            slideCode();
            return false;
        }
        ...
        if (gameWebView.getVisibility() == View.VISIBLE) {
            if (nc_sid.equals("") || nc_token.equals("") || nc_sig.equals("")) {
                ToastUtils.ToastUtils(getResources().getString(R.string.verify_err), BlackLoginActivity.this);
                return false;
            }
        }
        ...
        OkGo.<String>post(URLDecoder.decode(Constants.BaseUrl() + Constants.USERLOGIN + (Constants.ENCRYPT ? Constants.SIGN : "")))//
                .tag(this)//
                .upRequestBody(body)
                .execute(new NetDialogCallBack(this, true, BlackLoginActivity.this,
...
//                                EventBus.getDefault().postSticky(new MessageEvent("login_my"));
                                EvenBusUtils.setEvenBus(new Even(EvenBusCode.SHOWFRAMNET, "/user"));
                            } else {
//                                EventBus.getDefault().postSticky(new MessageEvent("login"));
                            ...
                            if (ConstantsData.passwod(mEtPassword)) {
                                ToastUtils.ToastUtils("您的密码过于简单，可能存在风险，请修改为复杂的密码", BlackLoginActivity.this);

                                Bundle build = new Bundle();
                                build.putInt("type", 0);
                                build.putBoolean("isBla", true);
                                FragmentUtilAct.startAct(BlackLoginActivity.this, new SafetyCenterFragB(false), build);
                            } else {
                                EvenBusUtils.setEvenBus(new Even(EvenBusCode.SHOWFRAMNET, "/home"));
                                finish();
                            }
                        } else if (li != null && li.getCode() != 0 && li.getMsg() != null) {
                            ToastUtils.ToastUtils(li.getMsg(), BlackLoginActivity.this);
                            slideCode();
                            if (gameWebView.getVisibility() == View.VISIBLE) {
                                nc_sid = "";
                                nc_token = "";
...
//                            count--;
    ...
    private void mdialog() {
        ...
        View inflate = LayoutInflater.from(BlackLoginActivity.this).inflate(R.layout.alertext_from, null);
        final EditText et = (EditText) inflate.findViewById(R.id.from_et);
        et.setHint(getResources().getString(R.string.google_code));
        mTDialog = new TDialog(BlackLoginActivity.this, TDialog.Style.Center, array, getResources().getString(R.string.google_code),
            ...
            public void onItemClick(Object object, int pos) {
                ...
                if (pos == 1) {
                    if (TextUtils.isEmpty(googleCode)) {
                        ToastUtils.ToastUtils(getResources().getString(R.string.google_code), BlackLoginActivity.this);
                    } else {
                        try {
                            questLogin();
                        } catch (UnsupportedEncodingException e) {
    ...
    private void setWebView() {
        ...
        gameWebView.setBackgroundColor(Color.parseColor("#00000000"));
        gameWebView.getBackground().setAlpha(0); // 设置填充透明度 范围：0-255
...
//                        nc_value = vb.getNc_value();
    ...
    private void hideBg(WebView view) {
