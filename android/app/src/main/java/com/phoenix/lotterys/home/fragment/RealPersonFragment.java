...
import com.phoenix.lotterys.home.bean.LotteryBean;
...
//        lotteryticketAdapter = new LotteryticketAdapter(real, getActivity());
        ...
        lotteryticketAdapter.setListener(new LotteryticketAdapter.OnClickListener() {
            ...
            public void onClickListener(View view, int position) {
                if(real.get(position).getIsPopup()==0){
                    ...
                    TDialog mTDialog = new TDialog(getActivity(), TDialog.Style.Center, array, hint_list[0],
                            hint_list[1]+ real.get(position).getTitle()+
                                    hint_list[2],""
                    ...
                    mTDialog.show();
                }else if(real.get(position).getIsPopup()==1){
                    Intent intent=new Intent();
                    intent.putExtra("id",real.get(position).getId());
                    intent.putExtra("name",real.get(position).getCategory());
                    intent.putExtra("title",real.get(position).getTitle());
...
//            startActivity(new Intent(getContext(), LoginActivity.class));
        ...
        OkGo.<String>get(URLDecoder.decode(Constants.BaseUrl()+Constants.GOTOGAMEREAL +String.format(UrlModel, SecretUtils.DESede(real.get(position).getId()),SecretUtils.DESede(token))+"&sign="+SecretUtils.RsaToken()))//
                .tag(this)//
