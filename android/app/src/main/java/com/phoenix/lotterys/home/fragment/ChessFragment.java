...
public class ChessFragment extends BaseFragment {
    ...
    public void initView(View view) {
        if (signViewPager != null)
            signViewPager.setObjectForPosition(view, 3);
            initRecyclerView();
    }
...
//        lotteryticketAdapter = new LotteryticketAdapter(card, getActivity());
        ...
        lotteryticketAdapter.setListener(new LotteryticketAdapter.OnClickListener() {
            ...
            public void onClickListener(View view, int position) {
                if(card.get(position).getIsPopup()==0){
                    ...
                    TDialog mTDialog = new TDialog(getActivity(), TDialog.Style.Center, array, hint_list[0],
                            hint_list[1]+ card.get(position).getTitle()+
                                    hint_list[2],""
                    ...
                    mTDialog.show();
                }else if(card.get(position).getIsPopup()==1){
                    Intent intent=new Intent();
                    intent.putExtra("id",card.get(position).getId());
                    intent.putExtra("name",card.get(position).getCategory());
                    intent.putExtra("title",card.get(position).getTitle());
...
//            startActivity(new Intent(getContext(), LoginActivity.class));
        ...
        OkGo.<String>get(URLDecoder.decode(Constants.BaseUrl()+Constants.GOTOGAMEREAL +String.format(UrlModel, SecretUtils.DESede(card.get(position).getId()),SecretUtils.DESede(token))+"&sign="+SecretUtils.RsaToken()))//
                .tag(this)//
