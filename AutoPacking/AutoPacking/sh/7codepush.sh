...
# 发布到测试环境（私钥签名）
code-push release-react $__Environment ios --t $__APPVersion --des "$__Version" -k $__KeyFile > /dev/null
# 发布到正式环境
#code-push release-react $__Environment ios --t $__APPVersion --dev false --d Production --des "$__Version" --m true
